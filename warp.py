...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    #SLIDE 5 IN VISUALIZED STEP PPT

    x_sphere = np.sin(xf)*np.cos(yf)
    y_sphere = np.sin(yf)
    z_sphere = np.cos(xf)*np.cos(yf)

    x_norm = (x_sphere/z_sphere)
    y_norm = (y_sphere/z_sphere)

    r_sq = (x_norm ** 2) + (y_norm ** 2)
    r_quad = (r_sq ** 2)

    xt = x_norm * (1 + k1 * r_sq + k2 * r_quad)
    yt = y_norm * (1 + k1 * r_sq + k2 * r_quad)
